...
class WeChatController extends Controller
{
    public function serve(Request $request)
    {
        ...
        $server->addEventListener('subscribe', function ($message) {
            ...
            $surplusAt = 0;
            $current = now();
            if (Carbon::parse($gptKey->end_at)->gt($current)) {
                $hours = Carbon::parse($gptKey->end_at)->diffInHours($current);
                $surplusAt = round($hours / 24, 1);
            }

            return "感谢关注!\n用户编号: $number\n身份口令：$gptKey->key\n会员剩余天数：$surplusAt 天\n1.验证请发送提示的：验证码\n2.查询信息请发送：信息\n3.加群请发送：加群\n4.续费请发送：续费\n5.免费系统：https://chat.wobcw.com\n6.会员系统：https://gpt.wobcw.com\n友情提示：新用户😀会赠送！赠送！赠送你 1 天会员系统体验时间,进入会员系统后要设置你的身份口令哦，快进入会员系统体验吧🤩！具体使用教程请查看公众号历史文章哦~";
        });
